...
function check_or_redirect(options) {
    ...
    return function(req,res,next){
        ...
        req.session = null;
        res.writeHead(307, {location: buildUri(options) + options.uris.login + '?' + querystring.stringify(query)});
        return res.end();
    }
}
...
function redirect(options) {
    ...
    return function(req,res,next){
        ...
        res.writeHead(307, {location: buildUri(options)+options.uris.login + '?' + querystring.stringify(queryopts)});
        return res.end();
    }
}
...
function logout(options) {
    ...
    return function(req,res,next){
        ...
        res.writeHead(307, {location: buildUri(options) + options.uris.logout + '?' + querystring.stringify(queryopts)});
        res.end()
    }
}
...
function ticket(options) {
    ...
    return function(req,res,next){
        ...
        var cas_uri = buildUri(options) + options.uris.serviceValidate + '?' +querystring.stringify(
                        {service:service,
                         ticket:ticket});
        ...
        request({uri:cas_uri}, function (error, response, body) {
            if (!error && response.statusCode == 200) {
                if(/cas:authenticationSuccess/.exec(body)){
                    ...
                    // strip the ticket
                    url.search = url.search.replace('ticket=' + url.query.ticket, '');
                    url.path = url.path.replace('ticket=' + url.query.ticket, '');
                    url.href = url.href.replace('ticket=' + url.query.ticket, '');
                    delete url.query.ticket;
                    res.writeHead(307, {location: buildUri(options) + formatUrl(url)});
                    res.end();
                } else {
                    logger.debug('auth failed');
                    res.writeHead(307, {location: buildUri(options) + options.uris.login + '?' + querystring.stringify({service: service})});
                    res.end();
                }
