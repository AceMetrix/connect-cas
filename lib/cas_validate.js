...
function ticket(options) {
    ...
    return function(req,res,next){
        ...
        if (ticket) {
            ...
            var validateUri = formatUrl(options) + options.uris.serviceValidate + '?' + querystring.stringify({service:service, ticket: ticket});
            validateTicket(validateUri, req, res, function(body, success){
                if (success) {
                    ...
                    if (req.session.st) {
                        ...
                        res.writeHead(307, {location: formatUrl(options) + options.uris.login + '?' + querystring.stringify({service: service})}); // this shoudl be the stripped params redir
                        res.end();
                    }
                }
            });
            ...
            if (options.gateway){
                if (req.session.gateway) {
                    ...
                    res.writeHead(307, {location: formatUrl(options) + options.uris.login + '?' + querystring.stringify({service: determine_service(req), gateway: true})});
                    res.end();
                }
                return;
            }
            ...
            if (req.session.st) {
                ...
                res.writeHead(307, {location: formatUrl(options) + options.uris.login + '?' + querystring.stringify({service: determine_service(req)})});
                res.end();
            }
        }
    }
