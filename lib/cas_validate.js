...
switch(((env.NODE_ENV+loglevel) || '').toLowerCase()){
    case 'production':
    production = true
    loglevel='warn'
...
console.debug('LOGLEVEL is',loglevel)
    ...
    return function(req,res,next){
        ...
        if(gateway){
            ...
            if(req.session.gateway !== undefined){
                console.debug('gateway already checked')
                return next()
            }
...
function invalidate(req,res,next){
    ...
    console.debug('handling ssoff')
    for( var param in req.body){
        if(/<samlp:SessionIndex>(.*)<\/samlp:SessionIndex>/.exec(req.body[param])){
            var st = RegExp.$1;
            console.debug("RECEIVED POST SSOFF REQUEST...INVALIDATING SERVICE TICKET "+st);
        ...
        request({uri:cas_uri}, function (error, response, body) {
            if (!error && response.statusCode == 200) {
                if(/cas:authenticationSuccess/.exec(body)){
                    ...
                    console.error('something else!' + body)
...
function force_protocol(url) {
    ...
    var result = urlp.host === undefined
               ? 'https://'+urlp.href
               : 'https://'+urlp.host
