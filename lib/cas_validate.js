...
// defaults
var defaults = {
    validation: '/cas/serviceValidate',
    login: '/cas/login'
};
...
function check_or_redirect(options){
    ...
    return function(req,res,next){
        ...
        var query = {}
        query.service = options.service || determine_service(req);
        if (options.gateway) query.gateway = options.gateway;

        console.log(querystring.stringify(query));
        logger.debug('no current session, redirecting to CAS server') ;

        res.writeHead(307, {location: options.host + options.path + '?' + querystring.stringify(query)});
        return res.end();
    }
}

function check_no_redirect(options){
    var options = _.extend(defaults, options, {gateway: true});
...
function redirect(options){
    ...
    var host = force_protocol(options.host)
    if (!host ) throw new Error('no CAS host specified');
    ...
    return function(req,res,next){
        ...
        res.writeHead(307, { 'location': host+options.path
                              +'?'
...
function ticket(options){
    ...
    return function(req,res,next){
        ...
        var cas_uri = host + options.validation
                    +'?'
