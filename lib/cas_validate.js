    ...
    return function validate(req,res,next){
        ...
        if(url.query && url.query.ticket){
            ...
            if(st = req.session.st){
                ...
                req.sessionStore.set(st,{'sid':req.sessionId,
                                         'lastAccess':+new Date});
            }else{
                // not okay.  must log in again
                console.log('ticket not in session store:'+JSON.stringify(req.session)) ;
                res.writeHead(403, { 'Content-Type': 'application/json' });
                var cas_login_uri =  'https://'+cas_host+login_service
                    +'?'
                    +querystring.stringify(
...
exports.invalidate = function invalidate(req,res,next){
    ...
    if(/<samlp:SessionIndex>(ST-.*-cas)<\/samlp:SessionIndex>/.exec(body)){
        var st = RegExp.$1;
